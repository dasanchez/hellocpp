// Fig. 17.14: This program reads a random-access file sequentially, updates
...
void updateRecord(fstream &updateFile)
{
    ...
    if (client.getAccountNumber() != 0)
    {
        ...
        double transaction; // charge of payment
        cin >> transaction;
...
void newRecord(fstream &insertInFile)
{
    ...
    if (client.getAccountNumber() == 0)
    {
        ...
        cerr << "Account #" << accountNumber << "already has information." << endl;
...
void deleteRecord(fstream &deleteFromFile)
{
    ...
    int accounNumber = getAccount("Enter account to delete");
    
    // move file-position pointer to correct record in file
    deleteFromFile.seekg((accounNumber - 1) * sizeof(ClientData));
    ...
    if (client.getAccountNumber() != 0)
    {
        ...
        // move file-position pointer to correct record in file
        deleteFromFile.seekp((accounNumber - 1) * sizeof(ClientData));

        // replace existing record with blank record
        deleteFromFile.write(reinterpret_cast<const char *>(&blankClient), sizeof(ClientData));

        cout << "Account #" << accounNumber << " deleted.\n";
    } // end if
    else // display error if record does not exist
        cerr << "Account #" << accounNumber << " is empty.\n";
} // end deleteRecord
...
// obtain account-nnumber value from user
int getAccount(const char * const prompt)
{
    int accountNumber;

    // obrain account-number value
    do
    {
        cout << prompt << " (1 - 100): ";
        cin >> accountNumber;
