...
int GradeBook::getMinimum()
{
    ...
    for (int grade = 0; grade < students; ++grade)
    {
        ...
        if (grades[grade] < lowGrade)
            lowGrade = grades[grade]; // new lowestGrade
    }   // end for
...
int GradeBook::getMaximum()
{
    ...
    for (int grade = 0; grade < students; ++grade)
    {
        // if ccurrent grade higher than highGrade, assign it to highGrade
        if (grades[grade] > highGrade)
            highGrade = grades[grade]; // new lowestGrade
    }                                  // end for

    return highGrade; // return lowest grade
} // end function getMaximum

// determine aver<ge grade for test
